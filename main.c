...
int WinMain(
  ...
  if (!SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2)) {OutputDebugStringA("WARNING: could not set DPI awareness");} // Support high-dpi screens
  // Register a "Window Class"
  static const wchar_t *window_class_name = L"WIN32_CUSTOM_TITLEBAR_EXAMPLE";
  WNDCLASSEXW window_class = {0};
  { window_class.cbSize       	= sizeof(window_class);
    window_class.lpszClassName	= window_class_name;
    window_class.lpfnWndProc  	= win32_custom_title_bar_example_window_callback; // Set the procedure that will receive window messages (events)
    window_class.style        	= CS_HREDRAW | CS_VREDRAW; // Ask to send WM_PAINT when resizing horizontally and vertically
  }; RegisterClassExW(&window_class);

  int window_style
    = WS_THICKFRAME   // required for a standard resizeable window (≝WS_SIZEBOX)
    | WS_SYSMENU      // Explicitly ask for the titlebar to support snapping via ❖◀ / ❖▶
...
static int
win32_dpi_scale(int value, UINT dpi) {return (int)((float)value * dpi / 96);}

static void
set_menu_item_state(HMENU menu, MENUITEMINFO* menuItemInfo, UINT item, bool enabled) {
  menuItemInfo->fState = enabled ? MF_ENABLED : MF_DISABLED;
  SetMenuItemInfo(menu, item, false, menuItemInfo);
}

// Adopted from: github.com/oberth/custom-chrome/blob/master/source/gui/window_helper.hpp#L52-L64
static RECT
win32_titlebar_rect(HWND handle) {
...
#define WIN32_FAKE_SHADOW_HEIGHT        	1 // Set this to 0 to remove the fake shadow painting
#define WIN32_MAXIMIZED_RECTANGLE_OFFSET	2 // The offset of the 2 rectangles of the maximized window button

static RECT
win32_fake_shadow_rect(HWND handle) {
  RECT rect;
  GetClientRect(handle, &rect);
...
win32_get_title_bar_button_rects(HWND handle, const RECT *title_bar_rect) {
  UINT dpi = GetDpiForWindow(handle);
...
static bool
win32_window_is_maximized(HWND handle) {
  WINDOWPLACEMENT placement = {0};
  placement.length = sizeof(WINDOWPLACEMENT);
  if (GetWindowPlacement(handle, &placement)) {return placement.showCmd == SW_SHOWMAXIMIZED;}
  return false;
}

static void
win32_center_rect_in_rect(RECT *to_center, const RECT *outer_rect) {
...
static LRESULT
win32_custom_title_bar_example_window_callback(HWND handle,UINT message,WPARAM w_param,LPARAM l_param) {
  ...
  switch (message) {
    ...
    // Info partially from: docs.microsoft.com/en-us/windows/win32/dwm/customframe#extending-the-client-frame
      // To remove the standard window frame, you must handle the WM_NCCALCSIZE message, specifically when its wParam value is TRUE and the return value is 0
    ...
    case WM_NCCALCSIZE: {
      ...
      if (win32_window_is_maximized(handle)) {requested_client_rect->top += padding;}

      return 0;
    }
    case WM_CREATE: { // Inform the application of the frame change to force redrawing with the new client area that is extended into the title bar
    ...
    case WM_NCHITTEST: { // Let the default procedure handle resizing areas
      ...
      if (title_bar_hovered_button == CustomTitleBarHoveredButton_Maximize) {return HTMAXBUTTON;}

      // Looks like adjustment happening in NCCALCSIZE is messing with the detection of the top hit area so manually fixing that.
      UINT dpi = GetDpiForWindow(handle);
      int frame_y = GetSystemMetricsForDpi(SM_CYFRAME, dpi);
      int padding = GetSystemMetricsForDpi(SM_CXPADDEDBORDER, dpi);
      ...
      if (cursor_point.y > 0 && cursor_point.y < frame_y + padding) {return HTTOP;}

      // Since we are drawing our own caption, this needs to be a custom test
      if (cursor_point.y < win32_titlebar_rect(handle).bottom) {return HTCAPTION;}
    ...
    case WM_PAINT: {
      ...
      { // Minimize button
        if (title_bar_hovered_button == CustomTitleBarHoveredButton_Minimize) {FillRect(hdc, &button_rects.minimize, title_bar_hover_brush);}
        RECT icon_rect  	= {0};
      ...
      { // Maximize button
        ...
        if (win32_window_is_maximized(handle)) { // Draw the maximized icon
          Rectangle(hdc,
      ...
      // Paint fake top shadow. Original is missing because of the client rect extension. You might need to tweak the colors here based on the color scheme of your app or just remove it if you decide it is not worth it.
    ...
    case WM_NCMOUSEMOVE: { // Track when mouse hovers each of the title bar buttons to draw the highlight correctly
      POINT cursor_point;
      ...
      if        (PtInRect(&button_rects.close   ,cursor_point))	{new_hovered_button = CustomTitleBarHoveredButton_Close;
      } else if (PtInRect(&button_rects.minimize,cursor_point))	{new_hovered_button = CustomTitleBarHoveredButton_Minimize;
      } else if (PtInRect(&button_rects.maximize,cursor_point))	{new_hovered_button = CustomTitleBarHoveredButton_Maximize;
      }
      if (new_hovered_button != title_bar_hovered_button) { // You could do tighter invalidation here but probably doesn't matter
    ...
    // If the mouse gets into the client area then no title bar buttons are hovered so need to reset the hover state
    case WM_MOUSEMOVE: {
      if (title_bar_hovered_button) {
        RECT title_bar_rect = win32_titlebar_rect(handle);
        InvalidateRect(handle, &title_bar_rect, FALSE); // You could do tighter invalidation here but probably doesn't matter
        SetWindowLongPtrW(handle, GWLP_USERDATA, (LONG_PTR)CustomTitleBarHoveredButton_None);
      }
      return DefWindowProc(handle, message, w_param, l_param);
    }
    case WM_NCLBUTTONDOWN: { // Handle mouse down and mouse up in the caption area to handle clicks on the buttons
      // Clicks on buttons will be handled in WM_NCLBUTTONUP, but we still need to remove default handling of the click to avoid it counting as drag.
      // Ideally you also want to check that the mouse hasn't moved out or too much between DOWN and UP messages.
      if (title_bar_hovered_button) {return 0;}
      return DefWindowProc(handle, message, w_param, l_param); // Default handling allows for dragging and double click to maximize
    }
    case WM_NCLBUTTONUP: { // Map button clicks to the right messages for the window
      if        (title_bar_hovered_button == CustomTitleBarHoveredButton_Close   ) {PostMessageW(handle, WM_CLOSE, 0, 0); return 0;
      } else if (title_bar_hovered_button == CustomTitleBarHoveredButton_Minimize) {ShowWindow  (handle, SW_MINIMIZE   ); return 0;
        ...
        int mode = win32_window_is_maximized(handle) ? SW_NORMAL : SW_MAXIMIZE;
                                                                                    ShowWindow  (handle, mode          ); return 0;}
      return DefWindowProc(handle, message, w_param, l_param);
    }
    case WM_NCRBUTTONUP: {
      if (w_param == HTCAPTION) {
        BOOL const isMaximized = IsZoomed(handle);
        MENUITEMINFO menu_item_info = {.cbSize = sizeof(menu_item_info),.fMask = MIIM_STATE};
        ...
        if (result != 0) {PostMessage(handle, WM_SYSCOMMAND, result, 0);}
      }
      return DefWindowProc(handle, message, w_param, l_param);
    }
    case WM_SETCURSOR: {SetCursor(LoadCursor(NULL, IDC_ARROW));break;} // Show an arrow instead of the busy cursor
